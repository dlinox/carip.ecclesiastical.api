...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('persons', function (Blueprint $table) {
            ...
            $table->string('paternal_name', 60)->nullable();
            $table->string('maternal_name', 60)->nullable();
            $table->enum('document_type', ['DNI', 'CE', 'PAS', 'RUC'])->default('DNI');
            $table->char('document_number', 16)->nullable()->check('LENGTH(document_number) >= 8');
            ...
            $table->fulltext(['name', 'paternal_name', 'maternal_name'], 'idx_full_name');
            // Foreign key
            $table->foreign('birth_country_id')->references('id')->on('countries')->onUpdate('cascade')->onDelete('cascade');
        });
    }
